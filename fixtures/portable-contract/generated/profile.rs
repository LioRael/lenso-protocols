...
impl __LensoIntoProfileCorpusRoundTripResult for Result<CorpusRoundTripResponse, lenso_plugin_authoring::PluginError<CorpusRoundTripError, RuntimeFailure>> {
    fn __lenso_into_result(self) -> Result<Result<CorpusRoundTripResponse, CorpusRoundTripError>, RuntimeFailure> {
        match self {
...
impl __LensoIntoProfileRoundTripResult for Result<RoundTripResponse, lenso_plugin_authoring::PluginError<RoundTripError, RuntimeFailure>> {
    fn __lenso_into_result(self) -> Result<Result<RoundTripResponse, RoundTripError>, RuntimeFailure> {
        match self {
            Ok(value) => Ok(Ok(value)),
            Err(lenso_plugin_authoring::PluginError::Domain(error)) => Ok(Err(error)),
...
macro_rules! __lenso_native_lower_profile {
    ($plugin:ty, $support:path) => {
        ...
        impl $crate::ProfileProvider for $plugin {
        fn corpus_round_trip(&self, context: __LensoNativeSupportProfile::InvocationContext, request: $crate::CorpusRoundTripRequest) -> __LensoNativeSupportProfile::NativeRequestFuture<$crate::ProfileCorpusRoundTrip> {
            ...
            ::std::boxed::Box::pin(async move {
                let result = <$plugin>::corpus_round_trip(&plugin, context, request).await;
...
#[doc(hidden)]
#[macro_export]
macro_rules! __lenso_native_endpoints_profile {
...
macro_rules! __lenso_native_provide_profile {
    ($provider:expr, $lifecycle:expr, $support:path) => {{
        ...
        let (request_endpoints, stream_endpoints, event_endpoints) =
            $crate::__lenso_native_endpoints_profile!($provider, $support);
